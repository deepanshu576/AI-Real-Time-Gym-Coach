#SQLite Setup (exercise repository, schema init, and CRUD basics)
...
@st.cache_resource  #@st.cache_resource use this decorator because this fnx call one time when the session start 

def _get_connection() -> sqlite3.Connection:
    conn = sqlite3.connect(_DB_PATH, check_same_thread=False)
    conn.row_factory = sqlite3.Row  # conn.row used beacause it return value in id name like format 
    return conn
